...
const GlobalDashboard = () => {
    ...
    // Initialize these with stable values to prevent dependency array changes
    ...
    const fetchAnalysisData = useCallback(async () => {
        ...
        try {
            ...
            if (failedResponses.length > 0) {
                console.warn("Some analytics requests failed:", failedResponses.map(r => r.status));
            }

            const [currentOverview, productPerformance, getCategoryStats] = await Promise.all(
                responses.map(async (response) => {
                    if (response.ok) {
                        return await response.json();
                    } else {
                        console.error(`Failed request to ${response.url}:`, response.status);
                        return { error: true };
                    }
                })
            );
            ...
            toast.error("Failed to load analytics data - please check server connection");
    ...
    }, [selectedYear, selectedMonth]); // Dependencies are now stable

    useEffect(() => {
        fetchAnalysisData();
    }, [fetchAnalysisData]); // Use the memoized function as dependency

    const fetchInvoices = useCallback(async () => {
        try {
            console.log("Attempting to fetch invoices from /api/products");
            ...
            if (res.ok) {
                ...
                console.log("Successfully fetched invoices:", invoices);
                setInvoices(invoices || []);
            } else {
                console.error("Failed to fetch invoice data. Status:", res.status);
            ...
            toast.error("Unable to load invoices. Please check if the server is running.");
        }
